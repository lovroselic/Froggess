...
const ENGINE = {
    ...
    resizeAndFill(CTX, pattern, targetSize) {
        if (!pattern) return; //ignore silently
    ...
    readMouse(event) {
        let canvasOffset;
        if (!event.data) {
            canvasOffset = $(ENGINE.topCanvas).offset();
        } else {
            canvasOffset = $(event.data.layer).offset();
        }

        const offsetX = canvasOffset.left;
        const offsetY = canvasOffset.top;
        const mouseX = parseInt(event.pageX - offsetX, 10);
        const mouseY = parseInt(event.pageY - offsetY, 10);
    ...
    */
    ...
    VECTOR2D: {
        ...
        drawBlock(monster, color = "#8000FF") {
            ...
            if (monster.moveState.pos.constructor === FP_Grid) {
                this._circle(CTX, monster.moveState.pos, monster.r, monster.moveState.dir, "#000");
            } else {
                this._circle(CTX, Vector3.to_FP_Grid(monster.moveState.pos), monster.r, Vector3.to_FP_Grid(monster.moveState.dir), "#000"); //
            }

            if (monster.moveState.boundingBox) {
                CTX.strokeStyle = color;
                CTX.lineWidth = 1;
